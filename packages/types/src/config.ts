...
// failure.
//
// Lifted verbatim from packages/collector/src/config.ts in sessions-spike
// WU v4-2 (design §3.5) so the v4 server can reuse it without deep-importing
// collector internals. Behaviour is unchanged from the collector original;
// the only edit is the `paths` import, which is now the same-package
// relative `./paths.js` instead of `@metaswarm-dashboard/types/paths`.
